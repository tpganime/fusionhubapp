...
const LiquidToggle: React.FC<{ checked: boolean; onChange: () => void; colorClass?: string }> = ({ checked, onChange, colorClass = 'bg-blue-500' }) => {
  return (
    <div 
      onClick={onChange}
      className={`liquid-switch ${checked ? 'active' : ''}`}
      style={{ backgroundColor: checked ? undefined : '' }} // Let CSS handle colors via classes for default
    >
       <div className={`liquid-switch-handle`} />
       {checked && <style>{`.liquid-switch.active { background-color: var(--tw-bg-opacity, 1) ${colorClass.replace('bg-', '')}; }`}</style>}
    </div>
  );
};
...
export const SettingsScreen: React.FC = () => {
  ...
  if (showPrivacyPolicy) {
    return (
      <div className={`h-full overflow-y-auto no-scrollbar gpu-accelerated ${enableAnimations ? 'animate-heavy-fade-in' : ''}`}>
  ...
  return (
    <div className={`h-full overflow-y-auto pb-32 no-scrollbar gpu-accelerated ${enableAnimations ? 'animate-heavy-fade-in' : ''}`}>
      <div className="sticky top-4 mx-4 z-50 glass-panel p-3 flex items-center mb-6">
        <button onClick={() => navigate('/profile')} className="p-2 rounded-full hover:bg-black/5 dark:hover:bg-white/10 transition-colors active:scale-90">
          <ArrowLeft className="w-6 h-6 text-gray-900 dark:text-white" />
        </button>
      ...
      <main className="px-5 space-y-6 max-w-md mx-auto">
        ...
        <section className={`${enableAnimations ? 'animate-slide-up-heavy' : ''}`} style={getAnimStyle(1)}>
          ...
          <div className="liquid-card overflow-hidden divide-y divide-gray-200/30 dark:divide-white/10">
             ...
             <div className="p-5 flex items-center justify-between">
               ...
               <LiquidToggle checked={theme === 'dark'} onChange={toggleTheme} colorClass="bg-indigo-600" />
            </div>
            ...
            <div className="p-5 flex items-center justify-between">
               ...
               <LiquidToggle checked={enableLiquid} onChange={toggleLiquid} colorClass="bg-cyan-500" />
            </div>
            
            {/* Transparency Slider */}
            {enableLiquid && (
               <div className="p-5 animate-slide-up-heavy">
            ...
            <div className="p-5 flex items-center justify-between">
               ...
               <LiquidToggle checked={enableAnimations} onChange={toggleAnimations} colorClass="bg-pink-500" />
            </div>
            ...
            <div className="p-5 flex items-center justify-between">
               ...
               <LiquidToggle checked={isNotificationGranted} onChange={enableNotifications} colorClass="bg-green-500" />
            </div>
          </div>
        </section>
        ...
        <section className={`${enableAnimations ? 'animate-slide-up-heavy' : ''}`} style={getAnimStyle(2)}>
          ...
          <div className="liquid-card overflow-hidden divide-y divide-gray-200/30 dark:divide-white/10">
            <div className="p-5 flex items-center justify-between">
               ...
               <LiquidToggle checked={currentUser.isPrivateProfile} onChange={togglePrivateProfile} colorClass="bg-purple-600" />
            </div>
            <div className="p-5 flex items-center justify-between">
               <div className="flex items-center gap-4">
               ...
               <LiquidToggle checked={currentUser.allowPrivateChat} onChange={togglePrivateChat} colorClass="bg-indigo-600" />
            </div>
          </div>
        </section>
        ...
        <p className={`text-center text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-8 opacity-50 ${enableAnimations ? 'animate-heavy-fade-in' : ''}`} style={{ animationDelay: '600ms' }}>FusionHub v1.3.4 LIQUID</p>
